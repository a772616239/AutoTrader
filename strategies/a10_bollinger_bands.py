...
class A10BollingerBandsStrategy(BaseStrategy):
    ...
    def _detect_bollinger_signal(self, symbol: str, data: pd.DataFrame,
        ...
        if prev_price <= prev_upper and current_price > current_upper:
            ...
            logger.info(f"🚀 {symbol} 布林带上轨突破 - 价格: {current_price:.2f}, 上轨: {current_upper:.2f}, 强度: {breakout_strength:.2f}, 置信度: {confidence:.2f}")
            ...
            return {
                ...
                'reason': f"布林带上轨突破: {current_price:.2f} > {current_upper:.2f}",
                'indicators': {
        ...
        # 下轨跌破信号 - 卖出
            ...
            logger.info(f"🔻 {symbol} 布林带下轨跌破 - 价格: {current_price:.2f}, 下轨: {current_lower:.2f}, 强度: {breakout_strength:.2f}, 置信度: {confidence:.2f}")
            ...
            return {
                ...
                'price': current_price,
                'confidence': confidence,
                'reason': f"布林带下轨跌破: {current_price:.2f} < {current_lower:.2f}",
                'indicators': {
                    'upper_band': current_upper,
                    'middle_band': current_middle,
                    'lower_band': current_lower,
    ...
    def check_exit_conditions(self, symbol: str, current_price: float,
        ...
        if price_change_pct <= stop_loss_pct:
            logger.warning(f"⚠️ {symbol} A10触发止损: 亏损{price_change_pct*100:.2f}%")
            return {
                'symbol': symbol,
        ...
        if price_change_pct >= take_profit_pct:
            logger.info(f"✅ {symbol} A10触发止盈: 盈利{price_change_pct*100:.2f}%")
            return {
                'symbol': symbol,
